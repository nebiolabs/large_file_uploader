function UploadPart(file, partNumber, upload) {
  ...
  this.ETag = '';
  this.stringToSign = function(){return 'PUT\n\nmultipart/form-data\n\nx-amz-date:' + this.upload.date + '\n/' + this.upload.config.bucket + '/' + encodeURI(upload.awsObjURL) + '?partNumber=' + this.partNumber + '&uploadId=' + this.upload.uploadId;};
  this.url = function(){return 'https://' + this.upload.config.bucket + '.s3.amazonaws.com/' + encodeURI(upload.awsObjURL) + '?partNumber=' + this.partNumber + '&uploadId=' + this.upload.uploadId;};
}
