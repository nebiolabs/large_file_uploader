function UploadPart(file, partNumber, upload) {
  ...
  this.ETag = '';
  this.stringToSign = function(){return 'PUT\n\ntext/plain;charset=UTF-8\n\nx-amz-date:' + this.upload.date + '\n/' + this.upload.bucket + '/' + encodeURI(this.file.name) + '?partNumber=' + this.partNumber + '&uploadId=' + this.upload.uploadId;};
  this.url = function(){return 'https://' + this.upload.bucket + '.s3.amazonaws.com/' + encodeURI(this.file.name) + '?partNumber=' + this.partNumber + '&uploadId=' + this.upload.uploadId;};
}
